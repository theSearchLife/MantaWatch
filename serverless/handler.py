...
# Class config (comma-separated, first = target) comes from the CLASSES job input.
CLASSES = []
TARGET_CLASS = ""
NON_TARGET_CLASS = ""
REPORT_CLASSES = []
...
def create_github_release(model_path: str, token: str, repo: str, tag: str, imgsz: int = 640) -> str:
    ...
    resp = requests.post(
        ...
        json={
            ...
            "body": f"YOLO11s-cls · {len(CLASSES)} classes: {' / '.join(CLASSES)}\n\nAuto-generated by RunPod training job.",
...
def _to_report_class(name: str) -> str:
    """Collapse the model's native classes to the 2 report classes: the target class
    stays, anything else becomes the non-target label.
    """
    return TARGET_CLASS if str(name).lower() == TARGET_CLASS else NON_TARGET_CLASS
...
def _check_dataset_classes(dataset_root, expected):
    """Fail if the dataset's train/ class folders don't match the configured classes."""
    train_dir = pathlib.Path(dataset_root, "train")
    found = sorted(d.name for d in train_dir.iterdir() if d.is_dir())
    if set(found) != set(expected):
        raise ValueError(
            f"Dataset classes {found} do not match configured CLASSES {sorted(expected)}; "
            "fix the Drive folders or the CLASSES variable so they match"
        )
    log(f"  Classes OK: {found}")


def handler(job: dict):
    inp = job.get("input", {})

    global CLASSES, TARGET_CLASS, NON_TARGET_CLASS, REPORT_CLASSES
    CLASSES = [c.strip() for c in (inp.get("classes") or "").split(",") if c.strip()]
    if len(CLASSES) < 2:
        raise ValueError(
            "CLASSES must list at least 2 comma-separated classes, target first "
            "(e.g. 'manta,other_fish,non_fish') — set the CLASSES repo variable"
        )
    ...
    try:
        ...
        _check_dataset_classes(dataset_root, CLASSES)
